...
const SelectReportTypeModal = ({
    ...
    getalltheme(THEME_LIST_PAYLOAD)
      .then((res) => {
        ...
        if (mode === "estimate") {
          ...
          list = list.filter((t) => t.nameEnglish === "Transportation Report");
        } else if (mode === "labour") {
          list = list.filter((t) => t.nameEnglish === "Labour Report");
        }
  ...
  const selectedFunctionLabel =
    selectedFunctionId == null
      ...
      : (eventData?.eventFunctions?.find(
          (f) => toId(f.id) === selectedFunctionId,
        )?.nameEnglish ?? "All Function");

  const eventDateTime = eventData?.eventStartDate
  ...
  const handleGenerate = (report) => {
    const hasReportKeys =
      Array.isArray(report.reportKeys) && report.reportKeys.length > 0;
        ...
        <div className="max-h-[75vh] overflow-y-auto px-6 py-5">
          ...
          <div className="mt-5 max-w-xs">
            ...
            <Select
              className="w-full"
              value={
                selectedFunctionId == null ? "all" : String(selectedFunctionId)
              }
              onChange={(val) =>
                setSelectedFunctionId(val === "all" ? null : toId(val))
              }
          ...
          <div className="mt-5 flex items-center gap-1 border-b border-gray-100">
            ...
            <div
              ref={tabStripRef}
              className="flex flex-1 gap-6 overflow-x-hidden px-1"
            >
              {loadingThemes ? (
                <span className="py-2.5 text-sm text-gray-400">
                  Loading themes…
                </span>
              ) : themes.length === 0 ? (
                <span className="py-2.5 text-sm text-gray-400">
