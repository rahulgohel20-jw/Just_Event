...
// This page only ever shows the outsource agency inventory category.
const INVENTORY_ITEMS_FILTER = 'OUTSOURCE';
// TODO: confirm the real rawCategoryId for Outsource Agency against
// RawItemMaster — this is a placeholder until confirmed (mirrors the other
// category pages' approach — 16 Flowers is the only one confirmed so far).
const OUTSOURCE_AGENCY_RAW_CATEGORY_ID = 24;
...
const handleImageFileSelected = (e) => {
  ...
  const fetchOutsourceAgencyCatalogItems = useCallback(
    async (query) => {
      try {
        const res = await getAllRawItemMaster({
          ...
          nameEnglish: query || '',
          page: 0,
          rawCategoryId: OUTSOURCE_AGENCY_RAW_CATEGORY_ID,
  ...
  images: extractImageUrls(item.images),
        ...
        console.error('Failed to fetch outsource agency catalog items:', err);
        return [];
      }
    },
    [userId]
  ...
  fetcher={fetchOutsourceAgencyCatalogItems}
  value={catalogPick}
