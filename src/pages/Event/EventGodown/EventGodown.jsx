...
// This page only ever shows the godown/stock inventory category.
const INVENTORY_ITEMS_FILTER = 'GODOWN';
// TODO: confirm the real rawCategoryId for Godown against RawItemMaster —
// this is a placeholder until confirmed (mirrors EventNewMaking's approach —
// 16 Flowers is the only one confirmed so far).
const GODOWN_RAW_CATEGORY_ID = 26;
...
const handleImageFileSelected = (e) => {
  ...
  const fetchGodownCatalogItems = useCallback(
    async (query) => {
      try {
        const res = await getAllRawItemMaster({
          ...
          nameEnglish: query || '',
          page: 0,
          rawCategoryId: GODOWN_RAW_CATEGORY_ID,
  ...
  images: extractImageUrls(item.images),
        ...
        console.error('Failed to fetch godown catalog items:', err);
        return [];
      }
    },
    [userId]
        ...
        <div className="rounded-2xl border border-primary-clarity bg-white p-6 shadow-sm">
          ...
          <div className="mt-4 flex flex-col gap-3 rounded-2xl bg-primary-lighest p-3 sm:flex-row sm:items-center">
            <div className="relative flex-1">
              ...
              <AsyncSearchSelect
                fetcher={fetchGodownCatalogItems}
                value={catalogPick}
