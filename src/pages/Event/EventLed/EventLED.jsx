...
// This page only ever shows the LED wall inventory category.
const INVENTORY_ITEMS_FILTER = 'LED_WALL';
// TODO: confirm the real rawCategoryId for LED Wall against RawCategoryMaster —
// placeholder until confirmed (16 was Flowers, 17 was Lighting's placeholder).
const LED_RAW_CATEGORY_ID = 18;
...
const EventLED = () => {
  ...
  const fetchLedCatalogItems = useCallback(
    async (query) => {
      try {
        const res = await getAllRawItemMaster({
          ...
          nameEnglish: query || '',
          page: 0,
          rawCategoryId: LED_RAW_CATEGORY_ID,
                    ...
                    {item.expanded && (
                      <div className="grid grid-cols-1 gap-6 border-t border-primary-clarity bg-white p-5 lg:grid-cols-2">
                        <div className="space-y-4">
                          ...
                          <div className="grid grid-cols-2 gap-3">
                            <div>
                              <Text type="secondary" className="block !text-xs font-semibold uppercase mb-2">L</Text>
                              <InputNumber style={{ width: '100%' }} min={0} value={item.length} onChange={(v) => updateItem(item.id, 'length', v ?? 0)} />
                            </div>
                            <div>
                              <Text type="secondary" className="block !text-xs font-semibold uppercase mb-2">B</Text>
                              <InputNumber style={{ width: '100%' }} min={0} value={item.breadth} onChange={(v) => updateItem(item.id, 'breadth', v ?? 0)} />
                            </div>
                          </div>
                          <div className="grid grid-cols-2 gap-3">
                            <div>
                              <Text type="secondary" className="block !text-xs font-semibold uppercase mb-2">H</Text>
                              <InputNumber style={{ width: '100%' }} min={0} value={item.height} onChange={(v) => updateItem(item.id, 'height', v ?? 0)} />
                            </div>
                            <div>
                          ...
                          <div className="rounded-xl border border-primary-clarity bg-primary-lighest p-4">
                            ...
                            <div className="mt-3 flex flex-col gap-1 border-t border-primary-clarity pt-3">
                              <div className="flex items-center justify-between">
                                ...
                                <span className="text-sm font-bold text-primary">Total Quantity: {item.qty}</span>
                              </div>
