...
// This page only ever shows the printing inventory category.
const INVENTORY_ITEMS_FILTER = 'PRINTING';
// TODO: confirm the real rawCategoryId for Printing against RawItemMaster —
// this is a placeholder until confirmed (mirrors the other category pages'
// approach — 16 Flowers is the only one confirmed so far).
const PRINTING_RAW_CATEGORY_ID = 22;
...
const handleImageFileSelected = (e) => {
  ...
  const fetchPrintingCatalogItems = useCallback(
    async (query) => {
      try {
        const res = await getAllRawItemMaster({
          ...
          nameEnglish: query || '',
          page: 0,
          rawCategoryId: PRINTING_RAW_CATEGORY_ID,
  ...
  images: extractImageUrls(item.images),
        ...
        console.error('Failed to fetch printing catalog items:', err);
        return [];
      }
    },
    [userId]
  ...
  fetcher={fetchPrintingCatalogItems}
  value={catalogPick}
                    ...
                    {item.expanded && (
                      <div className="grid grid-cols-1 gap-6 border-t border-primary-clarity bg-white p-5 lg:grid-cols-2">
                        <div className="space-y-4">
                          ...
                          <div className="grid grid-cols-2 gap-3">
                            <div>
                              <Text type="secondary" className="block !text-xs font-semibold uppercase mb-2">Price (₹ per sqft)</Text>
                          ...
                          <div className="rounded-xl border border-primary-clarity bg-primary-lighest p-4">
                            ...
                            <div className="mt-3 flex flex-col gap-1 border-t border-primary-clarity pt-3">
                              <div className="flex items-center justify-between">
                                ...
                                <span className="text-sm font-bold text-primary">Total Quantity: {combinedQty} {item.unit}</span>
                              </div>
