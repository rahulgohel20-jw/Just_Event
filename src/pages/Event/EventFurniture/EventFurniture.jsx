...
// This page only ever shows the furniture inventory category.
const INVENTORY_ITEMS_FILTER = 'FURNITURE';
// TODO: confirm the real rawCategoryId for Furniture against RawItemMaster —
// this is a placeholder until confirmed (mirrors the Lighting page's approach).
const FURNITURE_RAW_CATEGORY_ID = 21;
...
const handleImageFileSelected = (e) => {
  ...
  const fetchFurnitureCatalogItems = useCallback(
    async (query) => {
      try {
        const res = await getAllRawItemMaster({
          ...
          nameEnglish: query || '',
          page: 0,
          rawCategoryId: FURNITURE_RAW_CATEGORY_ID,
  ...
  images: extractImageUrls(item.images),
        ...
        console.error('Failed to fetch furniture catalog items:', err);
        return [];
      }
    },
    [userId]
  ...
  fetcher={fetchFurnitureCatalogItems}
  value={catalogPick}
