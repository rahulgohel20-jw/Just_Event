...
// This page only ever shows the mandap inventory category.
const INVENTORY_ITEMS_FILTER = 'MANDAP';
// TODO: confirm the real rawCategoryId for Mandap against RawCategoryMaster —
// placeholder until confirmed (16 Flowers, 17/18/19 Lighting/LED/Sound placeholders).
const MANDAP_RAW_CATEGORY_ID = 20;
...
const handleImageFileSelected = (e) => {
  ...
  setItems((prev) =>
    prev.map((item) => {
      if (item.id !== imageUploadItemId) return item;

      if (item.images?.[0]?.startsWith?.('blob:')) {
        URL.revokeObjectURL(item.images[0]);
      }
  ...
  const fetchMandapCatalogItems = useCallback(
    async (query) => {
      try {
        ...
        console.error('Failed to fetch mandap catalog items:', err);
        return [];
      }
    },
    [userId]
        ...
        <div className="rounded-2xl border border-primary-clarity bg-white p-6 shadow-sm">
          ...
          <div className="mt-4 flex flex-col gap-3 rounded-2xl bg-primary-lighest p-3 sm:flex-row sm:items-center">
            <div className="relative flex-1">
              ...
              <AsyncSearchSelect
                fetcher={fetchMandapCatalogItems}
                value={catalogPick}
