    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between bg-primary-inverse rounded-xl p-4">
        ...
        <button
          ...
          className={`w-11 h-6 rounded-full relative transition-colors shrink-0 ${addBrideGroom ? "bg-primary" : "bg-primary-clarity"
            }`}
        >
          <span
            className={`absolute top-0.5 left-0.5 w-5 h-5 rounded-full bg-white shadow transition-transform ${addBrideGroom ? "translate-x-5" : "translate-x-0"
              }`}
          />
        </button>
      </div>
...
function ClientCard({ index, client, showTypeToggle, onChange }) {
  ...
  const label = showTypeToggle ? index == 0 ? "PRIMARY CONTACT" : "SECONDARY CONTACT" : "";
  ...
  return (
    <div className="bg-light border border-primary-clarity rounded-2xl p-5">
      {showTypeToggle && (
        <div className="flex items-center gap-5 mb-5">
          ...
          <div>
            ...
            <p className="m-0 text-[12px] text-primary font-semibold">{label}</p>
          </div>
        </div>
      )}
      ...
      <div className={`grid grid-cols-1 ${showTypeToggle ? "grid-cols-1" : ""} sm:grid-cols-2 gap-4 mb-4`}>
        <div>
          <label className="block text-[13px] font-medium text-dark-light mb-1.5">
            Address
          </label>
          ...
          <div>
            ...
            <div className="flex gap-2">
              <button
                ...
                className={`flex-1 flex items-center justify-center gap-1.5 rounded-xl py-3 text-sm font-medium transition-colors ${!isBride
