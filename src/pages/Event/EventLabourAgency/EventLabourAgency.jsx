...
import { InputNumber, Select, Button, Typography, Empty, Spin } from "antd";
import {
  PlusOutlined,
  DeleteOutlined,
  CopyOutlined,
...
const AsyncSearchSelect = ({
  ...
  return (
    <Select
      ...
      onChange={(opt) =>
        onChange(
          opt ? { id: opt.value, label: opt.label } : { id: null, label: "" },
        )
      }
      notFoundContent={
        loading ? (
...
const buildItemsFromDetails = (
  ...
  details.map((row) => ({
    ...
    labourName: {
      id: row.partyId ?? null,
      label: labourNameMap[row.partyId] ?? "",
    },
    expanded: true,
    shifts: (row.labourShifts ?? []).map((s) => ({
      ...
      _uid: nextId(),
      shiftMaster: {
        id: s.shiftMasterId ?? null,
        label: shiftLabelMap[s.shiftMasterId] ?? "",
      },
      dateTime: s.dateTime ?? "",
      price: s.price ?? 0,
      qty: s.qty ?? 0,
...
const groupShiftsByDay = (shifts) => {
  ...
  entries.sort(([a], [b]) => {
    ...
    return (
      dayjs(a, DATE_ONLY_FORMAT).valueOf() -
      dayjs(b, DATE_ONLY_FORMAT).valueOf()
    );
  });
...
const applyStartTimeToDateTime = (currentDateTime, startTime, fallbackDate) => {
  ...
  const existing = currentDateTime
    ? dayjs(currentDateTime, DATE_TIME_FORMAT, true)
    : null;
  ...
  if (existing?.isValid()) {
    ...
    const fallback = fallbackDate
      ? dayjs(fallbackDate, DATE_TIME_FORMAT, true)
      : null;
    datePart = fallback?.isValid()
      ? fallback.format(DATE_ONLY_FORMAT)
      : dayjs().format(DATE_ONLY_FORMAT);
  }
...
const EventLabourAgency = () => {
  ...
  useEffect(() => {
    ...
    getbyeventid(eventId)
      .then((res) => {
        ...
        setFunctionOptions(
          fns.map((f) => ({ value: f.id, label: f.nameEnglish })),
        );

        if (fns.length > 0) {
          const matched = routeFunctionId
            ? fns.find((f) => String(f.id) === String(routeFunctionId))
            : null;
    ...
    try {
      ...
      return Object.fromEntries(
        (Array.isArray(list) ? list : []).map((v) => [
          v.id,
          v.nameEnglish || v.nameHindi || v.nameGujarati || "",
        ]),
      );
    } catch (err) {
  ...
  const fetchShiftLabelMap = useCallback(async () => {
    try {
      ...
      const content = body?.content ?? [];
      return Object.fromEntries(
        content.map((s) => [s.id, `${s.nameEnglish} · ${s.startTime}`]),
      );
    } catch (err) {
  ...
  const fetchRecordForFunction = useCallback(
    async (eventFunctionId) => {
      ...
      try {
        ...
        const details = Array.isArray(data.agenyDetails)
          ? data.agenyDetails
          : [];
        const [labourNameMap, shiftLabelMap, categoryLabelMap] =
          await Promise.all([
            fetchLabourNameMap(),
            fetchShiftLabelMap(),
  ...
  const toggleItemExpand = (itemUid) =>
    setItems((prev) =>
      prev.map((it) =>
        it._uid === itemUid ? { ...it, expanded: !it.expanded } : it,
      ),
    );

  const deleteItem = (itemUid) =>
    setItems((prev) => prev.filter((it) => it._uid !== itemUid));
      ...
      prev.map((it) => {
        ...
        let nextShifts = it.shifts.map((s) => {
          ...
          if (field === "shiftMaster" && value?.id && value?.startTime) {
            updated.dateTime = applyStartTimeToDateTime(
              s.dateTime,
              value.startTime,
              setUpDateTime,
            );
        ...
        if (!clearing && (field === "dateTime" || field === "shiftMaster")) {
          ...
          const conflict = nextShifts.find(
            (s) => s._uid !== shiftUid && shiftsConflict(s, changed),
          );
          if (conflict) {
            message.warning(
              "This party already has a shift at that exact date, time, and shift type.",
            );
  ...
  const addShift = (itemUid, dayKey) =>
    setItems((prev) =>
      prev.map((it) =>
        it._uid === itemUid
          ? { ...it, shifts: [...it.shifts, makeShift(dayKey)] }
          : it,
      ),
    );
  ...
  const duplicateShift = (itemUid, shiftUid) =>
    setItems((prev) =>
      prev.map((it) => {
        ...
        return {
          ...it,
          shifts: [...it.shifts, { ...src, id: null, _uid: nextId() }],
        };
      }),
    );
  ...
  const removeShift = (itemUid, shiftUid) =>
    setItems((prev) =>
      prev.map((it) =>
        it._uid === itemUid
          ? { ...it, shifts: it.shifts.filter((s) => s._uid !== shiftUid) }
          : it,
      ),
    );
  ...
  const handleSaveAll = async () => {
    if (!selectedFunction?.id) {
      showApiError(
        { message: "Select a function before saving." },
    ...
    if (!incharge) {
      showApiError(
        { message: "Production Incharge is required." },
        { title: "Missing Production Incharge" },
      );
      return;
    }
    ...
    if (items.length === 0) {
      showApiError(
        { message: "Add at least one labour allocation before saving." },
          ...
          <div className="flex justify-between">
            <div className="flex flex-wrap items-center gap-3 pb-4 border-b border-gray-100">
              ...
              <Select
                ...
                onSearch={(q) =>
                  fetchFunctionOptions(q).then((res) =>
                    setFunctionOptions(
                      res.map((o) => ({ value: o.id, label: o.label })),
                    ),
                  )
                }
          ...
          <div className="mt-4 flex flex-col gap-3 rounded-2xl bg-primary-lighest p-3 sm:flex-row sm:items-center">
            ...
            <div className="relative flex-1">
              <Select
                ...
                value={
                  catalogPick?.id
                    ? { value: catalogPick.id, label: catalogPick.label }
                    : undefined
                }
                onChange={(opt) =>
                  setCatalogPick(
            ...
            <div className="mt-2 space-y-3">
              {items.map((item) => {
                ...
                return (
                  <div
                    key={item._uid}
                    className="rounded-xl border border-primary-clarity overflow-hidden"
                  >
                    <div className="grid grid-cols-2 gap-3 bg-primary-lighest px-4 py-4 sm:grid-cols-[2fr_1.4fr_1fr_1fr_auto] sm:items-center">
                      ...
                      <span className="font-bold text-primary sm:text-center">
                        {currency(totalQty)}
                      </span>
                      <span className="font-bold text-primary sm:text-center">
                        {currency(totalPrice)}
                      ...
                      <div className="flex items-center gap-2 justify-end col-span-2 sm:col-span-1">
                        ...
                        <Button
                          ...
                          size="small"
                          icon={
                            item.expanded ? <UpOutlined /> : <DownOutlined />
                          }
                    ...
                    {item.expanded && (
                      <div className="space-y-4 border-t border-primary-clarity bg-white p-5">
                        ...
                        {groupShiftsByDay(item.shifts).map((day) => (
                          <div
                            key={day.dateKey}
                            className="rounded-xl border border-primary-clarity bg-primary-lighest/40 p-4"
                          >
                                  ...
                                  >
                                    ...
                                    <div className="flex items-center justify-end gap-3 col-span-2 sm:col-span-1">
                                      <Button
                                        ...
                                        icon={<DeleteOutlined />}
                                        onClick={() =>
                                          removeShift(item._uid, shift._uid)
                                        }
                            ...
                            <Button
                              type="primary"
                              icon={<PlusOutlined />}
                              onClick={() => addShift(item._uid, day.dateKey)}
