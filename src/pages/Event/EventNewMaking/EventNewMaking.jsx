...
// This page only ever shows the new making inventory category.
const INVENTORY_ITEMS_FILTER = 'NEW_MAKING';
// TODO: confirm the real rawCategoryId for New Making against RawItemMaster —
...
const NEW_MAKING_RAW_CATEGORY_ID = 25;
...
const handleImageFileSelected = (e) => {
  ...
  const fetchNewMakingCatalogItems = useCallback(
    async (query) => {
      try {
        const res = await getAllRawItemMaster({
          ...
          nameEnglish: query || '',
          page: 0,
          rawCategoryId: NEW_MAKING_RAW_CATEGORY_ID,
  ...
  images: extractImageUrls(item.images),
        ...
        console.error('Failed to fetch new making catalog items:', err);
        return [];
      }
    },
    [userId]
        ...
        <div className="rounded-2xl border border-primary-clarity bg-white p-6 shadow-sm">
          ...
          <div className="mt-4 flex flex-col gap-3 rounded-2xl bg-primary-lighest p-3 sm:flex-row sm:items-center">
            <div className="relative flex-1">
              ...
              <AsyncSearchSelect
                fetcher={fetchNewMakingCatalogItems}
                value={catalogPick}
