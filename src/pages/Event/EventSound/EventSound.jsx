...
// This page only ever shows the sound inventory category.
const INVENTORY_ITEMS_FILTER = 'SOUND';
// TODO: confirm the real rawCategoryId for Sound against RawCategoryMaster —
// placeholder until confirmed (16 Flowers, 17 Lighting, 18 LED — all placeholders except Flowers).
const SOUND_RAW_CATEGORY_ID = 19;
...
const handleImageFileSelected = (e) => {
  ...
  const fetchSoundCatalogItems = useCallback(
    async (query) => {
      try {
        const res = await getAllRawItemMaster({
          ...
          nameEnglish: query || '',
          page: 0,
          rawCategoryId: SOUND_RAW_CATEGORY_ID,
  ...
  images: extractImageUrls(item.images),
        ...
        console.error('Failed to fetch sound catalog items:', err);
        return [];
      }
    },
    [userId]
  ...
  fetcher={fetchSoundCatalogItems}
  value={catalogPick}
