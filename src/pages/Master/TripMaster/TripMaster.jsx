...
import { GetAllTrip, DeleteTrip } from "@/services/apiServices";
...
const TripMaster = () => {
  ...
  const handleView = (record) => console.log("View trip:", record);
  ...
  const handleDelete = async (record) => {
    const confirmed = await confirmDelete(record.tripName?.english || "this trip");
    if (!confirmed) return;

    try {
      const res = await DeleteTrip(record.id);
      showApiResult(res, {
        successTitle: "Trip Deleted",
        onSuccess: fetchTripList,
      });
    } catch (err) {
  ...
  const filteredData = useMemo(() => {
    return tableData.filter((row) => {
      const englishName = row.tripName?.english || "";
      const matchesSearch = englishName.toLowerCase().includes(searchText.toLowerCase());
