...
import { addupdatemenutype, Translateapi } from "@/services/apiServices";
...
const Addmenuitemcattype = ({ open, onClose, onSave, initialData }) => {
  ...
  const handleTranslate = async (englishText) => {
    try {
      ...
      console.error("Failed to translate menu type name:", err);
      return null;
    }
  };
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const res = await addupdatemenutype(payload);

      const success = showApiResult(res, {
        successTitle: isEditMode ? "Menu Type Updated" : "Menu Type Saved",
      ...
      console.error("Failed to save menu type:", err);
      showApiError(err, {
        title: "Failed",
      });
    } finally {
      ...
      footer={
        <div className="flex justify-between items-center px-6 pb-6">
          ...
          >
            ...
            {isEditMode ? "Update Menu Type" : "Save Menu Type"}
          </button>
        </div>
      }
    >
        ...
        <div className="flex justify-between items-start mb-5">
          <div>
            <h2 className="text-xl font-bold text-primary">
              {isEditMode ? "Edit Menu Type" : "Add Menu Type"}
            </h2>
            <p className="text-sm text-gray-500 mt-1">
              Create or update a menu type used to classify menu categories
              and items.
            </p>
          </div>
          <button
