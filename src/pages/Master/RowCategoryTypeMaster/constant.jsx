...
export const getRowMaterialTypeColumns = ({ onEdit, onDelete }) => [
    {
        accessorKey: "srNo",
        header: "Sr. No.",
        cell: ({ row }) => (
    ...
    },
    {
        ...
        cell: ({ row }) => <p className="">{row.original.nameEnglish}</p>,
    },
    {
        accessorKey: "status",
        header: "Status",
        cell: ({ row }) => {
            ...
            return (
                <span
                    ...
                    ${active ? "bg-success-lighter text-success" : "bg-dark-lighter text-dark-clarity"}`}
                >
                    <span className={`h-2 w-2 rounded-full ${active ? "bg-success-light" : "bg-dark-clarity"}`} />
